...
TOOLS: list[Tool] = [
    ...
    Tool(
        ...
        description="Generate the full Yaam directory tree in the project. If already initialized and force=false, returns a confirmation request.",
        input_schema={
            "type": "object",
            "properties": {
                "project_path": {"type": "string", "description": "Path to the project root"},
...
def _is_yaam_initialized(project_path: str) -> bool:
    return (Path(project_path) / "contexts").is_dir()


def _check_writable(*paths: Path) -> str | None:
    for path in paths:
        try:
            if path.exists():
                ...
                if not os.access(parent, os.W_OK):
                    return f"Permission denied: cannot create {path} (parent not writable)"
...
async def _setup_yaam_framework(project_path: str, project_name: str, stack: str = "generic", dry_run: bool = False, force: bool = False) -> str:
    ...
    if _is_yaam_initialized(project_path) and not force:
        return json_dumps({
            ...
            "message": "Yaam is already initialized in this project. Set force=true to overwrite, or choose a different project_path.",
            "hint": "Ask the user if they want to overwrite the existing Yaam framework.",
        })
    ...
    for dir_name, files in base_structure.items():
        ...
        err = _check_writable(target_dir)
        if err:
